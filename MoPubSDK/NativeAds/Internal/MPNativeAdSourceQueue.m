...
#import "MPNativeAdSourceQueue+Logger.h"
...
- (void)replenishCache
{
    ...
    [adRequest startForAdSequence:self.currentSequence withCompletionHandler:^(MPNativeAdRequest *request, MPNativeAd *response, NSError *error) {
        if (response && !error) {
            [self if_logSuccessAdAttemptWithLoadingBeganTime:loadingBeganTime];
			...
			[self if_logFailedAdAttemptWithLoadingBeganTime:loadingBeganTime];
